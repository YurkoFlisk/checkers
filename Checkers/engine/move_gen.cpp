...
template<colour TURN, move_type MT>
void MoveGenEnglish::_get_all_moves(std::vector<Move>& vec) const
{
	// Search for capture-moves first
	if (MT != NON_CAPTURE)
	{
		for (int i = 0; i < board.piece_count[turn_simple(TURN)]; ++i)
		{
			...
			Move move(Path(1, pos));
			move.set_original(board[pos.row][pos.column]);
			_find_deep_capture<TURN>(vec, move, pos.row, pos.column);
		}
		for (int i = 0; i < board.piece_count[turn_queen(TURN)]; ++i)
		{
			const auto& pos = board.piece_list[turn_queen(TURN)][i];
...
template<colour TURN>
void MoveGenEnglish::_find_deep_capture(std::vector<Move>& vec, Move& move, int8_t row, int8_t column) const
{
	...
	for (size_t dir = 0; dir < 2; ++dir)
	{
		...
		{
			_find_deep_capture<TURN>(vec, move, r2, c2);
			if (old == vec.size()) // If in recursive call we haven't found any move, then 'move' is a final capture and is one of possible captures
			{
				move.set_become(Piece(TURN == WHITE ? WHITE_SIMPLE : BLACK_SIMPLE));
				vec.push_back(move);
...
template<colour TURN>
void MoveGenEnglish::_find_deep_capture_queen(std::vector<Move>& vec, Move& move, int8_t row, int8_t column, bool(&captured)[8][8]) const
{
	static constexpr int d_row[4] = { 1, 1, -1, -1 }, d_column[4] = { 1, -1, 1, -1 };
	for (size_t dir = 0; dir < 4; ++dir)
	{
...
template void	MoveGenEnglish::_find_deep_capture<WHITE>(std::vector<Move>&, Move&, int8_t, int8_t) const;
template void	MoveGenEnglish::_find_deep_capture<BLACK>(std::vector<Move>&, Move&, int8_t, int8_t) const;
template void	MoveGenEnglish::_find_deep_capture_queen<WHITE>(std::vector<Move>&, Move&, int8_t, int8_t, bool(&)[8][8]) const;
template void	MoveGenEnglish::_find_deep_capture_queen<BLACK>(std::vector<Move>&, Move&, int8_t, int8_t, bool(&)[8][8]) const;
template void	MoveGenEnglish::_get_all_moves<WHITE, ALL>(std::vector<Move>&) const;
template void	MoveGenEnglish::_get_all_moves<WHITE, CAPTURE>(std::vector<Move>&) const;
template void	MoveGenEnglish::_get_all_moves<WHITE, NON_CAPTURE>(std::vector<Move>&) const;
template void	MoveGenEnglish::_get_all_moves<BLACK, ALL>(std::vector<Move>&) const;
template void	MoveGenEnglish::_get_all_moves<BLACK, CAPTURE>(std::vector<Move>&) const;
template void	MoveGenEnglish::_get_all_moves<BLACK, NON_CAPTURE>(std::vector<Move>&) const;